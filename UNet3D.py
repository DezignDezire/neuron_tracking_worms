...
def double_conv(in_c, out_c):
    conv = nn.Sequential(
        nn.Conv3d(in_c, out_c, kernel_size=(3, kernel_xy_size, kernel_xy_size), padding=1),
        nn.ReLU(inplace=True),
        nn.Conv3d(out_c, out_c, kernel_size=(3, kernel_xy_size, kernel_xy_size), padding=1),
        nn.ReLU(inplace=True)
    )
    return conv
...
class UNet3D(nn.Module):
    ...
    def __init__(self, n_out):
        ...
        self.up_trans_1 = nn.ConvTranspose3d(n_chan_4, n_chan_3, kernel_size=(2, kernel_xy_size, kernel_xy_size), stride=2)
        self.up_conv_1 = double_conv(n_chan_4, n_chan_3)
        self.up_trans_2 = nn.ConvTranspose3d(n_chan_3, n_chan_2, kernel_size=(3, kernel_xy_size, kernel_xy_size), stride=2)
        self.up_conv_2 = double_conv(n_chan_3, n_chan_2)
        self.up_trans_3 = nn.ConvTranspose3d(n_chan_2, n_chan_1, kernel_size=(2, kernel_xy_size, kernel_xy_size), stride=2)
        self.up_conv_3 = double_conv(n_chan_2, n_chan_1)

        # TODO: do not use kernel size of 10 but kernel size=1
        self.out = nn.Conv3d(n_chan_1, 2, kernel_size=(10, 1, 1))

        self.fc = nn.Linear(2 * 325 * 450, n_out)
