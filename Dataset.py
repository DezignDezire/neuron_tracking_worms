...
## use image_shape to normalize coordiantes of centers between [0, 1]
...
def rotate_image(image, positions, angle):
...
class SupervisedDataset(object):
  ...
  def __init__(self, mode, batch_size):
    ...
    self.img_data = zarr.open('C:/Users/ZimAdmin/Documents/bright_worm5/dat/2021-12-17_16-28-19_worm5-channel-0-pco_camera1bigtiff_preprocessed.zarr.zip')
    ...
    self.n_neurons = len(finished_neurons)
    
    # ## y_positions shape: neuron1_x, neuron1_y, neuron1_z, neuron2_x, neuron2_y,...
    # try:
    #   y_pos = torch.load('y_pos_worm5')
    #   self.y_positions = y_pos
    #   print("loaded stored positions")
    # except:
    self.y_positions = get_ground_truth_coordinates(traces[:2], finished_neurons, self.n_neurons, self.img_data[0].shape)
    n_samples = 2
      # torch.save(self.y_positions, 'y_pos_worm5')
      # print("created positions from traces file")

    np.random.seed(1220)
    indices = np.random.permutation(n_samples)
    if mode == 0:   # TRAIN
      self.indxs = indices[:int(n_samples*0.2)]
      # self.indxs = indices[:300]
    elif mode == 1: # VAL
      self.indxs = indices[int(n_samples*0.7):int(n_samples*0.85)]
      # self.indxs = indices[400:450]
    elif mode == 2: # TEST
      # self.indxs = indices[int(n_samples*0.85):int(n_samples*0.85)+2]
      self.indxs = np.arange(2)
    else:
  ...
  def __getitem__(self, idx):
    ...
    # image, positions = rotate_image(image, positions, 20)
